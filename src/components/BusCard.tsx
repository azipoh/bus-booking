 ...
 * Shows operator info, timings, amenities, fare, and a book button.
 */
import { Link } from 'react-router-dom';
import { Clock, Star, MapPin, Wifi, Plug, Utensils, Monitor } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import type { Bus, Route, Schedule } from '@/data/mockData';
import { motion } from 'framer-motion';

interface BusCardProps {
  schedule: Schedule & { bus: Bus; route: Route };
  index: number;
}
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="text-center">
            <p className="font-heading text-xl font-bold text-foreground">{schedule.departureTime}</p>
          ...
          <div className="text-center">
            <p className="font-heading text-xl font-bold text-foreground">{schedule.arrivalTime}</p>
        ...
        <div className="flex flex-col items-start gap-2 lg:items-center">
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm font-semibold text-foreground">{bus.rating}</span>
            <span className="text-xs text-muted-foreground">({bus.reviews})</span>
          </div>
